...
const CardAuthorBox2: FC<CardAuthorBox2Props> = ({ className = '', author }) => {
  const { displayName, handle = '/', avatarUrl, jobName, count, bgImage, location } = author
  return (
    <Link
      href={'/authors/' + handle}
    ...
    >
      ...
      <div className="relative flex -translate-y-7 flex-col items-center px-6 pt-[1px] text-center">
        ...
        <div className="mt-6">
          ...
          {location && <span className={`mt-1 block text-sm text-neutral-500 dark:text-neutral-400`}>{location}</span>}
        </div>
      </div>
    </Link>
  )
